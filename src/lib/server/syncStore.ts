...
export class SyncStore {
	...
	async listHistory(accountId: string, before?: number, slot?: string): Promise<HistoryPage> {
		...
		).rows as unknown as Array<{ historyId: number; savedAt: number }>;
	...
	async listProfileHistory(accountId: string, before?: number): Promise<ProfileHistoryPage> {
		...
		).rows as unknown as Array<{ savedAt: number }>;
	...
	async getProfileSnapshot(
		...
		await this.db.ready;
		const point = await this.relay.execute({
		...
		const rows = (
			await this.relay.execute({
				sql: `SELECT id, slot, ciphertext FROM envelopes WHERE account_id = ? AND created_at <= ? AND slot > ?
			UNION ALL SELECT id, slot, ciphertext FROM envelope_history
			WHERE account_id = ? AND created_at <= ? AND saved_at > ? AND slot > ?
			ORDER BY slot LIMIT ?`,
				args: [accountId, at, after, accountId, at, at, after, HISTORY_PAGE_SIZE + 1]
			})
		).rows as unknown as HistoryEnvelope[];
		const envelopes = rows.slice(0, HISTORY_PAGE_SIZE);
		return {
			envelopes,
			nextAfter: rows.length > HISTORY_PAGE_SIZE ? envelopes.at(-1)!.slot : null
		};
	}
	...
	async purgeExpiredHistory(now = Date.now()): Promise<number> {
		...
		return (
			await this.relay.execute({
				sql: 'DELETE FROM envelope_history WHERE saved_at < ?',
				args: [now - HISTORY_TTL_MS]
			})
		).rowsAffected;
	}
	...
	async sync(
		...
		const result = await withTxn(this.relay, async (tx) => {
			...
			const currentBySlot = new Map<string, EncryptedEnvelope & { createdAt: number }>();
			for (const slotBatch of chunk(slots)) {
				const placeholders = slotBatch.map(() => '?').join(', ');
				const rows = (
					await tx.execute({
						sql: `SELECT seq, id, ciphertext, slot, created_at AS createdAt FROM envelopes
						 WHERE account_id = ? AND slot IN (${placeholders})`,
						args: [accountId, ...slotBatch]
					})
				).rows as unknown as Array<EncryptedEnvelope & { createdAt: number }>;
			...
			for (const deletion of deletions) {
				...
				deletionStatements.push(
					...
					{
						sql: `INSERT INTO envelope_history(account_id, slot, id, ciphertext, created_at, saved_at)
							VALUES (?, ?, ?, ?, ?, ?)`,
						args: [
							accountId,
							removed.slot,
			...
			for (const upload of uploads) {
				...
				if (prior)
					uploadStatements.push({
						sql: `INSERT INTO envelope_history(account_id, slot, id, ciphertext, created_at, saved_at)
						VALUES (?, ?, ?, ?, ?, ?)`,
						args: [accountId, prior.slot, prior.id, prior.ciphertext, prior.createdAt, deletedAt]
					});
				uploadStatements.push({
					sql: `INSERT INTO envelopes(account_id, slot, seq, id, ciphertext, created_at)
					VALUES (?, ?, ?, ?, ?, ?)
					...
					args: [accountId, upload.slot, sequence, upload.id, upload.ciphertext, deletedAt]
				});
				if (prior) knownIds.delete(prior.id);
				knownIds.add(upload.id);
				currentBySlot.set(upload.slot, { ...upload, seq: sequence, createdAt: deletedAt });
				envelopeCount = projectedCount;
				ciphertextBytes = projectedBytes;
				added = true;
			}
			await executeInBatches(tx, uploadStatements);
			if (added || deletionStatements.length > 0) {
				await tx.execute({
					sql: 'INSERT INTO profile_history_points(account_id, saved_at) VALUES (?, ?)',
					args: [accountId, deletedAt]
				});
				const history = (
					await tx.execute({
						sql: 'SELECT history_id AS historyId, length(ciphertext) AS bytes, saved_at AS savedAt FROM envelope_history WHERE account_id = ? ORDER BY history_id DESC',
						args: [accountId]
					})
				).rows as unknown as Array<{ historyId: number; bytes: number; savedAt: number }>;
				let retainedBytes = 0;
				let floor = 0;
				for (const [index, row] of history.entries()) {
					retainedBytes += row.bytes + ENVELOPE_STORAGE_OVERHEAD_BYTES;
					if (
						index >= HISTORY_MAX_ENTRIES ||
						row.savedAt < deletedAt - HISTORY_TTL_MS ||
						retainedBytes > maxAccountBytes
					) {
						floor = Math.max(floor, row.savedAt);
						await tx.execute({
				...
				await tx.execute({
					...
					args: [accountId, deletedAt - HISTORY_TTL_MS, floor]
				});
			}
