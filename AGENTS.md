...
- Keep named reusable `css`, `cva`, and `sva` contracts in `panda/styles.ts` and import them through `$panda/styles`; this is the single registry while the design system settles. Keep recipe variant selections and truly one-off adjustments next to the markup that owns them, and split the registry only when clear component boundaries emerge.
