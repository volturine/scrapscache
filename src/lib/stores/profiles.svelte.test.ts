...
describe('replacing the sync key of a lost device', () => {
	...
	function scenario(overrides: { remoteRecords?: number; localRecords?: number } = {}) {
		const previous = workspace('rotating', true);
		...
		return { previous, registered, deleted };
	}

	it('moves the workspace to a new key in place and deletes the account the old one opened', async () => {
		const { previous, deleted } = scenario();
		await putNote(previous.id, note('rotated-note'));
		...
		const [rotated] = syncStore.profiles;
		expect(syncStore.profiles).toHaveLength(1);
		expect(rotated.id).toBe(previous.id);
		expect(rotated.syncKey).not.toBe(previous.syncKey);
		expect(readProfiles()[0].syncKey).toBe(rotated.syncKey);
		expect(syncStore.account?.syncKey).toBe(rotated.syncKey);
		expect(await noteIds(previous.id)).toEqual(['rotated-note']);
	});

	it('puts the old key back when the new account came up short', async () => {
		...
		expect(outcome).toMatchObject({ ok: false, step: 'confirm', replacementRemoved: true });
		// The replacement is torn down; the account being replaced is untouched.
		const deletedKeys = deleted.mock.calls.map(([account]) => account.syncKey);
		expect(deletedKeys).toHaveLength(1);
		expect(deletedKeys).not.toContain(previous.syncKey);
		expect(syncStore.profiles).toEqual([previous]);
		expect(readProfiles()).toEqual([previous]);
		expect(syncStore.account?.syncKey).toBe(previous.syncKey);
	});

	it('puts the old key back even when the relay will not delete the new account', async () => {
		const { previous, deleted } = scenario({ localRecords: 4, remoteRecords: 3 });
		deleted.mockResolvedValue(false);

		const outcome = await new ProfileCoordinator().replaceSyncKey({
			exportBackup: async () => undefined
		});

		expect(outcome).toMatchObject({ ok: false, step: 'confirm', replacementRemoved: false });
		expect(syncStore.profiles).toEqual([previous]);
		expect(syncStore.account?.syncKey).toBe(previous.syncKey);
	});

	it('refuses when this device has not finished syncing, and does not register anything', async () => {
		const { previous, registered } = scenario();
		...
		expect(outcome).toMatchObject({ ok: false, step: 'preflight' });
	...
	it('backs up before it registers anything', async () => {
		...
		const order: string[] = [];
		registered.mockImplementation(async () => {
			order.push('register');
			return Response.json({ error: 'stop here' }, { status: 503 });
		});
	...
	it('carries the human check through to registering the new account', async () => {
		const { registered } = scenario();
		registered.mockResolvedValue(Response.json({ error: 'stop here' }, { status: 503 }));
		...
		// The relay gates registration on Turnstile, and rotation is a registration.
		const [url, init] = registered.mock.calls[0];
		expect(url).toBe('/api/sync/register');
		expect(JSON.parse(String(init?.body)).turnstileToken).toBe('turnstile-token');
	});
