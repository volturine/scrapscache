...
| History API     | `src/routes/api/sync/history/`                           | Owner-only encrypted prior versions             |
...
Each accepted sync change creates a profile checkpoint. The relay records when
each opaque envelope entered and left the profile, so a device can reconstruct
the complete encrypted profile at a checkpoint. The gallery shows a read-only
historical profile, supports restoring selected notes as copies, and can replace
the synced profile across devices. The note editor lists older versions of that
note and restores one as a copy. The relay retains checkpoints for up to 30 days;
storage is capped at 500 old envelopes or the account's byte quota. If an old
envelope is removed, checkpoints that need it are removed too. Historical data
is decrypted in memory and is never written into IndexedDB. The daily retention
sweep and account deletion remove expired history and its ciphertext.
